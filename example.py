...
def pipeLine(rawCOFITS, rmsFITS=None, averageRMS=0.5, processPath="./"):
    ...
    print(f"Label fits name: {doMWdbscan.getLabelFITSName(True)}")
...
if 0:
    ...
    doMWdbscan.produceCloudIntFITS("Q1Sub.fits", "Q1SubdbscanS2P4Con1_Clean.fits", "Q1SubdbscanS2P4Con1_Clean.fit",
                                   outputPath="./cloudIntPath", foreground=True)
...
if 0:  # an example for only produceIndividualCLouds
    ...
    # cloudSubCubes
    doMWdbscan.produceIndividualClouds(doMWdbscan.rawCOFITS, doMWdbscan.getLabelFITSName(doClean=True),
                                       doMWdbscan.getSaveCatName(doClean=True))


def catalog_from_label_fits():
    doMWdbscan.labelFITSName = r'13_Clean_by_12.fits'
    doMWdbscan.getCatFromLabelArray(doClean=True)  # by cleaning, we remove noise clusters
